		...
		<?php
			...
			if(isset($_POST['Submit']))
			{
				...
				for($i=1;$i<=$no_of_pass;$i++)
				{
					...
					{
						$query="SELECT count(*) from Customer c WHERE c.name=?  and c.customer_id=f.customer_id";
						$stmt=mysqli_prepare($dbc,$query);
					...
					if($_POST['pass_meal'][$i-1]=='yes')
						$total_no_of_meals++;
					mysqli_stmt_bind_param($stmt,"ississs",$i,$pnr,$_POST['pass_name'][$i-1],$_POST['pass_age'][$i-1],$_POST['pass_gender'][$i-1],$_POST['pass_meal'][$i-1]);
					mysqli_stmt_execute($stmt);
					$affected_rows=mysqli_stmt_affected_rows($stmt);
